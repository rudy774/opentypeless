...
8. Require OpenTypeless `0.1.1` or newer for AppSumo cloud-word usage, because older clients do not send operation metadata.
...
1. `CloudLlmProvider` sends a backward-compatible request body:
   ```json
   {
     ...
     "context": {
       ...
       "stageKey": "018f9a9b-7c3e-7b1a-a2f3-2e1b8a6a8f10:llm",
...
5. `stageKey` is stage-specific and deterministic per operation:
   - STT uses `${operationId}:stt`
   - LLM uses `${operationId}:llm`
6. `requestType` values are `voice_pipeline`, `stt_only`, or `llm_only`.
7. `clientVersion` is the package version sent as a string. The AppSumo launch client version is `0.1.1`.
8. `CloudSttProvider` sends multipart text fields alongside `audio`:
   - `operationId`
   - `stageKey`
   - `requestType`
   - `expectsCloudLlm`
   - `clientVersion`
9. Rust `AppType` must be explicitly mapped to lower-case strings: `email`, `chat`, `code`, `document`, `general`.
10. Tests cover the serialized LLM context and STT multipart metadata for selected-text, non-selected-text, and STT-only flows.
...
6. `client_upgrade_required` shows:
   - `Update OpenTypeless to use AppSumo cloud words.`
...
2. A paid-cloud plan is active only when status has:
   - `source: "creem"` with an active Pro entitlement, or
   - `source: "appsumo"`, `licenseStatus: "active"`, and a positive `cloudWordsLimit`.
...
6. If TalkMore returns `minimumDesktopVersion` above the current app version, onboarding explains that an update is required.
7. Tests cover Free, Pro, AppSumo Tier 1, refunded AppSumo, pending AppSumo, unknown plan responses, and upgrade-required responses.
...
3. Unknown plan IDs should not automatically unlock cloud.
4. AppSumo cloud access is active only when `source === 'appsumo'`, `licenseStatus === 'active'`, `cloudWordsLimit > 0`, and the current app version is at least `minimumDesktopVersion` when that field is present.
5. Creem cloud access is active only when `source === 'creem'`, `plan === 'pro'`, and the subscription is active or canceled-but-not-expired according to TalkMore.
6. `plan === 'pro'` must not be the only paid check. Add a helper like `hasManagedCloudAccess(status, appVersion)`.
7. If AppSumo status is otherwise active but `minimumDesktopVersion` is greater than the current app version, show upgrade-required copy and do not run cloud provider tests.
...
9. `errors.client_upgrade_required`: `Update OpenTypeless to use AppSumo cloud words.`
10. `settings.cloudLlmActive`: `Cloud AI is active for your plan.`
11. `settings.cloudSttActive`: `Cloud speech recognition is active for your plan.`
...
2. Add `PlanId`, `PlanSource`, and `hasManagedCloudAccess(status, appVersion)` helper.
...
4. Unit test pending AppSumo does not unlock cloud.
5. Unit test active AppSumo below `minimumDesktopVersion` returns upgrade required.
...
### Phase 4: Cloud operation metadata
...
3. Add deterministic stage keys: `${operationId}:stt` and `${operationId}:llm`.
4. Serialize app type, selected-text state, translation state, request type, client version, and length metrics.
5. Add STT multipart metadata fields: `operationId`, `stageKey`, `requestType`, `expectsCloudLlm`, and `clientVersion`.
6. Do not include secrets or duplicate raw user text in the metadata object.
7. Keep server compatibility by preserving `messages` and `stream`.
...
4. Rust unit test that STT multipart sends `operationId`, `stageKey`, `requestType`, `expectsCloudLlm`, and `clientVersion`.
5. Rust unit test that STT-only flow uses `requestType: "stt_only"` and no LLM stage key.
6. Regression test that ordinary spoken commands remain content outside selected-text mode.
...
4. Show upgrade-required copy when TalkMore returns `minimumDesktopVersion` greater than the current app version.
5. Add tests for cloud onboarding with Free, Pro, AppSumo, refunded AppSumo, pending AppSumo, quota-exhausted, and upgrade-required responses.
...
7. `client_upgrade_required` shows update-required copy, not an auth or quota error.
8. Selected-text rewrite works and ordinary transcription does not execute spoken commands.
9. Cloud onboarding and connection tests pass for AppSumo plans without requiring API keys.
...
| Old clients cannot send operation metadata | AppSumo cloud words cannot be settled once | Require OpenTypeless `0.1.1` for AppSumo cloud usage and show update-required copy from TalkMore status. |
